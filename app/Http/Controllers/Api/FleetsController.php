...
class FleetsController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $fleets = Fleets::query();
            $fleets =  $fleets->select('id','driver_id','vehicle_id','status','updated_at')->with('user',function($q){
    ...
    {
        try {
            $fleets = Fleets::where('id',$id)->select('id','driver_id', 'vehicle_id','status','updated_at')->with('user',function($q){
                $q->select('id','first_name','last_name','email','phone');
            })
            ->with('vehicle',function($q){
                $q->select('id','number','type');
