...
class SeatTypeController extends Controller
{
    ...
    {
        // Module Data
        $this->title = trans_choice('seat_type', 1);
        $this->route = 'admin.seat-type';
        $this->view = 'admin.seat-type';
        $this->path = 'seat-type';
        $this->access = 'seat-type';
    ...
    try{  
        ...
        $data['rows'] = $seattypes->orderBy('id', 'desc')->get();
    ...
    public function store(Request $request)
    {
        try{
            ...
            foreach ($names as $name) {
                ...
                if (SeatType::where('name', $name)->exists()) {
             ...
             foreach ($names as $name) {
                 //check record in DB
                $existName = SeatType::where('name', $name)->first();
                if(!$existName){
    ...
    public function destroy(SeatType $seatType)
    {
        try{
            if($seatType){
                ...
                $associated = Student::where('seat_type_id',$seatType->id)->first();
                if($associated){
