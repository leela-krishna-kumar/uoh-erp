                            ...
                            <table id="basic-table" class="display table nowrap table-striped table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('Fat') }}</th>
                                        <th>{{ __('BMI') }}</th>
                                        <th>{{ __('RM(Resting Metabolism)') }}</th>
                                        <th>{{ __('VF(Visceral Fat)') }}</th>
                                         {{-- <th>{{ __('Sports') }}</th> --}}
                                        <th>{{ __('Action') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                  @foreach($rows as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{ @$row->payload['age']}}</td>
                                        <td>{{ @$row->payload['weight']}}</td>
                                        <td>{{ @$row->payload['height']}}</td>
                                        <td>{{ @$row->payload['fat']}}</td>
                                        <td>{{ @$row->payload['bmi']}}</td>
                                        <td>{{ @$row->payload['rm']}}</td>
                                        <td>{{ @$row->payload['vf']}}</td>
                                         {{-- <td>
                                             @foreach ($row->sports_ids as $sports_id)
                                                {{ getSportsNameById($sports_id) }} @if (!$loop->last), @endif
                                             @endforeach
                                         </td>  --}}
