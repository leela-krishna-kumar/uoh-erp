<div class="card-block" id="inputFormField">
      <div class="row">
            <div class="form-group col-md-2">
                  ...
                  <select onchange="getTeachers(this)" class="form-control select2" name="subject[]" id="subject" required>
                        <option value="">{{ __('select') }}</option>
                        @foreach( $subjects as $subject )
                              <option value="{{ $subject->id }}" data-managed_by="" @if(old('subject') == $subject->id) selected @endif>{{ $subject->code }} - {{ $subject->title }}</option>
                        @endforeach
                  </select>
            ...
            <div class="form-group col-md-2">
                  ...
                  <select class="form-control select2 teacher-select" name="teacher[]" id="teacher" required>
                        <option value="">{{ __('select') }}</option>
            ...
            <div class="form-group col-md-2">
                  ...
                  <select class="form-control select2" name="room[]" id="room" required>
                        ...
                        <option value="{{ $room->id }}" @if(old('room') == $room->id) selected @endif>{{ $room->title }} - {{ $room->type }}</option>
                        @endforeach
                  </select>
