            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <ul class="nav nav-pills mb-3 card-block" id="myTab" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link @if($weekday == 3) active @endif text-uppercase" id="day{{ $weekday }}-tab" data-bs-toggle="tab" href="#day{{ $weekday }}" role="tab" aria-controls="day{{ $weekday }}" aria-selected="true">
                                @if( $weekday == 1 )
                                {{ __('day_saturday') }}
                                @elseif( $weekday == 2 )
                                    {{ __('day_sunday') }}
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        @foreach($weekdays as $weekday)
                        <div class="tab-pane fade @if($weekday == 3) show active @endif" id="day{{ $weekday }}" role="tabpanel" aria-labelledby="day{{ $weekday }}-tab">
                            <div class="">
                                <div class="row">
                                    <div class="col-md-12">
...
<script type="text/javascript">
    ...
    //Get Teachers on change in Subject
    function getTeachers(selectElement) {
        var selectedSubjectId = $(selectElement).val();
        var managedBy = $(selectElement).find('option:selected').data('managed_by');
        ...
        $.ajax({
            ...
            success: function(data) {
                var teacherSelect = $(selectElement).closest('.row').find('.teacher-select');
                teacherSelect.empty();
                
                // Populate the teacher select element with the retrieved data.
                $('.teacher-select').append('<option value="">{{ __("Select") }}</option>');
