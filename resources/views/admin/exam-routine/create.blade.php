                ...
                <div class="card">
                    ...
                    <div class="card-block">
                        ...
                        @method('PUT')
                            <div class="row">
                                @php 
                                    $teacherIds = $row->subject->managed_by;
                                    if (is_array($teacherIds)) {
                                        $subkectTeachers = App\User::whereIn('id', $teacherIds)->get();
                                    } else {
                                        $teacherIdsArray = explode(',', str_replace(['[', ']', '"'], '', $teacherIds));
                                        $subkectTeachers = User::whereIn('id', $teacherIdsArray)->get();
                                    }
                                @endphp 
...
@section('page_js')
    <script>
        function getTeacher(key){
            let subject = $('.subject'+key).val();
            $.ajax({
                type:'GET',
                url: "{{ route('admin.get-teachers') }}",
                data:{
                    subject:subject
                },
                success: function(response) {
                    $('option', '.teacherSelect'+key).remove();
                    $('.teacherSelect'+key).html(response.html);
                }
            });  
        }
    </script>
