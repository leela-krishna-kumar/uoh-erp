<div class="form-group col-md-3">
      ...
      <select class="form-control select2 subject0" onchange="getTeacher(0)" name="subject" id="subject" required>
            <option value="">{{ __('select') }}</option>
            @foreach( $subjects as $subject )
            <option value="{{ $subject->id }}" @if(old('subject') == $subject->id) selected @endif>{{ $subject->code }} - {{ $subject->title }} 
                  (
                        @if( $subject->class_type == 1 )
                        {{ __('class_type_theory') }}
                        @elseif( $subject->class_type == 2 )
...
<div class="form-group col-md-3">
      ...
      <select class="form-control select2 teacherSelect0" name="teachers[]"  multiple required>
...
<div class="form-group col-md-3">
      ...
      <select class="form-control select2" name="rooms[]" id="" multiple required>
