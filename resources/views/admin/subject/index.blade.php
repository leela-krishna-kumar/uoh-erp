                        ...
                        <form class="needs-validation" novalidate method="get" action="{{ route($route.'.index') }}">
                            <div class="row gx-2">
                                ...
                                <div class="form-group col-md-2 ">
                                    ...
                                    <select multiple  class="form-control select2"  name="regulation_ids[]">
                                        @foreach($regulations as $regulation)
                                            <option value="{{ $regulation->id }}" @if(request()->regulation_ids && (in_array($regulation->id, request()->regulation_ids))) selected @endif>{{ $regulation->name }}</option>
                                        @endforeach
                                    </select>
                                    <div class="invalid-feedback">
                        ...
                        <div class="table-responsive">
                            <table id="basic-table" class="display table nowrap table-striped table-hover" style="width:100%">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th>{{ __('field_subject_name') }}</th>
                                        <th>{{ __('field_code') }}</th>
                                        <th>{{ __('field_credit_hour_short') }}</th>
                                        <th>{{ __('field_subject_type') }}</th>
                                        <th>{{ __('field_class_type') }}</th>
                                        <th>{{ __('field_chapter') }}</th>
                                        <th>{{ __('field_program') }}</th>
                                        <th>{{ __('field_regulation') }}</th>
                                        <th>{{ __('field_group') }}</th>
                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            {{@App\Models\Subject::CLASS_TYPES[$row->class_type]['label']}}
                                            <!-- @if( $row->class_type == 1 )
                                        ...
                                        <td>
                                            <a href="{{route($lesson_route.'.index',['subject_id'=>$row->id])}}">{{$row->lessons->count() ?? 0}}</a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if ($row->regulation_ids)
                                                @foreach ($row->regulation_ids as $regulation_id)
                                                <span class="badge badge-info">
                                                    {{getRegulationName($regulation_id)}}
                                                </span>
                                                @endforeach
                                            @endif
                                        </td>
