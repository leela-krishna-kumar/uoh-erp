            ...
            <div class="col-sm-12">
                <div class="card">
                    ...
                    <div class="card-block">
                        <form class="needs-validation" novalidate method="get" action="{{ route('admin.hostel.staff-fee-defaulters') }}">
                            <div class="row gx-2">
                            ...
                            <table id="basic-table" class="display table nowrap table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('Fee to be Paid') }}</th>
                                        <th>{{ __('field_status') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    @if(getStaffFeeByType($row,'Hostel',request()->category)['remaining_fee'] > 0)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $row->id }}</td>
                                        ...
                                        <td>{{ number_format((float)getStaffFeeByType($row,'Hostel',request()->category)['remaining_fee'], 2, '.', '') }} </td>
                                        <td>
