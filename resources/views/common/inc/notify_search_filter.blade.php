...
{{-- <div class="form-group col-md-2">
  <label for="section">{{ __('Target Audience') }} <span>*</span></label>
	<select class="form-control targetAudience" name="targetAudience" id="targetAudience">
		<option value="0">{{ __('all') }}</option>
		@if(isset($targetAudience))
		@foreach( $targetAudiences->sortBy('title') as $targetAudience )
		<option value="{{ $targetAudience->id }}" @if( old('targetAudience') == $targetAudience->id) selected @endif>{{ $targetAudience->title }}</option>
		@endforeach
		@endif
	</select>

	<div class="invalid-feedback">
		{{ __('required_field') }} {{ __('Target Audience') }}
	</div>
</div> --}}
