                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>
                                            @if( $row->status == 1 )
                                            <span class="badge badge-pill badge-success">{{ __('status_open') }}</span>
                                            @else
                                            <span class="badge badge-pill badge-danger">{{ __('status_close') }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="dropdown show d-inline-block">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="statusMenuLink">
                                                    <a class="dropdown-item" href="#" onclick="document.getElementById('status_open_{{ $row->id }}').submit();">{{ __('status_open') }}</a>
                                                    <a class="dropdown-item" href="#" onclick="document.getElementById('status_close_{{ $row->id }}').submit();">{{ __('status_close') }}</a>
                                                </div>

                                                <form action="{{ route($route.'.status', $row->id) }}" method="post" id="status_open_{{ $row->id }}">
                                                    @csrf
                                                    <input type="hidden" name="status" value="1">
                                                </form>
                                                <form action="{{ route($route.'.status', $row->id) }}" method="post" id="status_close_{{ $row->id }}">
                                                    @csrf
