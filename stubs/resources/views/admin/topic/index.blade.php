                    ...
                    <div class="card-block">
                        <form class="needs-validation" novalidate method="get" action="{{ route($route.'.index') }}">
                            <div class="row gx-2">
                                ...
                                <div class="form-group col-md-3">
                                    <label for="status">Chapter</label>
                                    <select class="form-control" name="chapter_id" id="chapter_id">
                                        <option value=""selected>{{ __('Select Chapter') }}</option>
                                        @foreach ($chapters as $key => $chapter)
                                            <option value="{{$chapter->id}}" @if(request()->get('chapter_id') == $chapter->id ) selected @endif>{{ $chapter->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <tbody>
                                  @foreach( $rows as $key => $row )
                                    <tr>
                                        ...
                                        <td>{{ $row->end_date ?? '' }}</td>
                                        <td>
                                            <span class="badge badge-{{ App\Models\Topic::STATUSES[$row->status]['color'] }}">{{ App\Models\Topic::STATUSES[$row->status]['label'] }}</span>
                                        </td>
                                        <td>
                                            @can($access.'-edit')
